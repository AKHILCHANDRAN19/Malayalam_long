...
# --- Resource Paths ---
# ** Updated sound file paths for alternating effects **
sound_file_whoosh = os.path.join(input_folder, "Whoos 3.mp3") # Sound for ODD clips (1, 3, ...)
sound_file_mouse = os.path.join(input_folder, "Mouse.mp3")   # Sound for EVEN clips (2, 4, ...)
fire_overlay_video_path = os.path.join(input_folder, "Fire.mp4") # Path to fire overlay video
FINAL_OUTPUT_FILENAME = "final_output_with_alternating_sounds.mp4" # Updated name for clarity
...
if not os.path.exists(input_folder):
    try:
        ...
        print(f"Please ensure your {num_background_videos} background MP4s (1.mp4, 2.mp4, ...), Fire.mp4, Whoos 3.mp3, and Mouse.mp3 are in this folder.")
    except OSError as e:
...
if not os.path.exists(sound_file_whoosh): # Check Whoosh sound
    print(f"ERROR: Sound effect file not found: {sound_file_whoosh}")
    essential_files_exist = False
if not os.path.exists(sound_file_mouse): # Check Mouse sound
    print(f"ERROR: Sound effect file not found: {sound_file_mouse}")
    essential_files_exist = False
...
if not essential_files_exist:
     print("Essential base files (fire video, background videos, Whoos 3.mp3, Mouse.mp3) or FFmpeg missing. Please check paths and files. Exiting.")
     exit()
# --- End Check ---
...
# =====================================================
# Part 1: Create Clips with DYNAMIC Duration, Overlay, Animation, Fire, EFFECT SOUND & TTS
# =====================================================
...
for clip_info in final_clip_mapping:
    ...
    segment_num = final_clip_idx + 1 # 1, 2, 3... (This is the clip number for alternating sound)
    ...
    # *** Determine which sound effect to use based on segment_num ***
    if segment_num % 2 != 0: # Odd numbered clips (1, 3, 5...)
        current_effect_sound_path = sound_file_whoosh
        effect_sound_name = "Whoos 3.mp3"
    else: # Even numbered clips (2, 4, 6...)
        current_effect_sound_path = sound_file_mouse
        effect_sound_name = "Mouse.mp3"
    ...
    print(f" -> Using Effect Sound: {effect_sound_name}") # ** Indicate which sound is used **
    ...
    if not os.path.exists(sentence_tts_path):
         ...
         continue
    if not os.path.exists(current_effect_sound_path): # ** Check the selected effect sound **
         print(f"ERROR: Effect sound file not found: {current_effect_sound_path}. Skipping segment {segment_num}.")
         continue
    ...
    # --- 3. Merge Video, EFFECT SOUND, and TTS Part ---
    if silent_clip_ok and split_success:
        print(f"  Step 3: Merging video, {effect_sound_name}, and TTS part -> {os.path.basename(final_clip_path)}")
        ...
        merge_ok = False
        try:
            # FFmpeg command to mix Effect Sound and TTS part, map video, limit duration
            merge_cmd = [
                ffmpeg_executable, "-y",
                "-i", silent_clip_path,              # Input 0: Silent Video
                "-i", current_effect_sound_path,     # Input 1: Correct Effect Sound (Whoosh or Mouse)
                "-i", current_tts_part_path,         # Input 2: Correct TTS part
                "-filter_complex",
                # Mix the effect sound (Input 1) and the TTS part (Input 2)
                # Play both from the start, let the video duration cut them off
                "[1:a][2:a]amix=inputs=2:duration=longest:dropout_transition=1[a_mix]",
            ...
            success, _ = run_ffmpeg_command(merge_cmd, f"Video/Audio Merge ({effect_sound_name})")
